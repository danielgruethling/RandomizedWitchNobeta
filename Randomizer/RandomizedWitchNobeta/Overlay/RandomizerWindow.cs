    ...
    private void ShowRandomizerWindow()
    {
        ...
        TabBar("MainTabBar", () =>
        {
            TabItem("Randomizer", () =>
            {
                ...
                if(ImGui.CollapsingHeader("Archipelago Multiworld Connector", ImGuiTreeNodeFlags.DefaultOpen))
                {
                    ImGui.Checkbox("Randomize using Archipelago", ref settings.Archipelago);
                    WithDisabled(!settings.Archipelago, () =>
                    {
                        ImGui.InputText("Hostname", ref settings.ArchipelagoHostname, 256, ImGuiInputTextFlags.EscapeClearsAll);
                        ImGui.InputInt("Port", ref settings.ArchipelagoPort);
                        ImGui.InputText("Slot Name", ref settings.ArchipelagoSlotName, 256, ImGuiInputTextFlags.EscapeClearsAll);
                        ImGui.InputText("Password", ref settings.ArchipelagoPassword, 256, ImGuiInputTextFlags.EscapeClearsAll | ImGuiInputTextFlags.Password);
                        ImGui.NewLine();
                        if(ButtonColored(PrimaryButtonColor, "Connect to Archipelago"))
                        {
                            //Connect to Archipelago
                            var apSettings = ArchipelagoConnector.ConnectAP($"{settings.ArchipelagoHostname}:{settings.ArchipelagoPort}", settings.ArchipelagoSlotName, settings.ArchipelagoPassword);

                            //Retrieve settings from Archipelago
                            if (apSettings != null)
                            {

                                // Apply settings to keep references
                                settings.Apply(apSettings);
